...
@Component
public class GovFeePayTask {
    @Autowired
    view_govPayRepository govPayRep;
    @Autowired
    ...
    Logger logger= LoggerFactory.getLogger(GovFeePayTask.class);
    @Scheduled(cron = "0 0 4 * * ?")
    @Transactional
    public void Process() {
        String CollectionName = "govpay";
        try {
            Integer  XSource =Math.toIntExact(govPayRep.count());
            ...
            if(XSource.equals(XNow)==false) {
                ...
                logger.info("govpay集合没有发生变化。不进行数据操作!");
                return ;
            }
            Long X1=System.currentTimeMillis();
            int Total = Math.toIntExact(XSource) /50000 + 1;
            logger.info("开始同步tbgovPay表,一共{}条记录，分成{}次同步.",XSource,Total);
            for (int i = 0; i < Total; i++) {
                Long T1=System.currentTimeMillis();
                List<view_goypay> Os= govMapper.getAll(i*50000,50000);
                if (Os.size() > 0) {
