...
public class complexExcelBuilder {
    ...
    public byte[] getContentSender(Map<String, Object> content, String ConsType,
            Map<String, List<Map<String, Object>>> cache) throws Exception {
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        writeSingle(content);
        if (content.containsKey("Rows")) {
            String rowText = content.get("Rows").toString();
            ...
            List<Map<String, Object>> rows = JSON.parseObject(rowText, O);
            writeRowsSender(rows, ConsType, cache);
        }
        HSSFFormulaEvaluator.evaluateAllFormulaCells(book);
        book.write(outputStream);
        byte[] Bx = outputStream.toByteArray();
    ...
    private void writeRowsSender(List<Map<String, Object>> rows, String ConsType,
            Map<String, List<Map<String, Object>>> cache) throws Exception {
        int startRow = firstRowIndex;
        int count = 0;
        List<Map<String, Object>> listFinalMap = new ArrayList<>();
        if (ConsType.equals("download")) {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");

            for (int i = 0; i < cache.size(); i++) {
                Map<String, Object> row = rows.get(i);
                String AppNo = row.get("SHENQINGH").toString();
                List<Map<String, Object>> listMaps = cache.get(AppNo);
                for (int j = 0; j < listMaps.size(); j++) {
                    Map<String, Object> listMap = listMaps.get(j);
                    Map<String, Object> finalMap = new HashMap<>();
                    Iterator iter = listMap.entrySet().iterator();
                    while (iter.hasNext()) {
                        Map.Entry mapKeys = (Map.Entry) iter.next();
                        if (mapKeys.getKey().equals("PAYSTATE")) {
                            finalMap.put("PAYSTATE", mapKeys.getValue());
                        }
                        if (mapKeys.getKey().equals("MONEY")) {
                            finalMap.put("MONEY", mapKeys.getValue());
                        }
                        finalMap.put("FAMINGRXM", row.get("FAMINGRXM"));
                        finalMap.put("SHENQINGR", row.get("SHENQINGR"));
                        if (mapKeys.getKey().equals("JIAOFEIR")) {
                            finalMap.put("JIAOFEIR", DateUtils.format(sdf.parse(mapKeys.getValue().toString()), "yyyy-MM-dd"));
                        }
                        finalMap.put("ZHUANLIMC", row.get("ZHUANLIMC"));
                        if (mapKeys.getKey().equals("COSTNAME")) {
                            finalMap.put("COSTNAME", mapKeys.getValue());
                        }
                        finalMap.put("SHENQINGRXM", row.get("SHENQINGRXM"));
                        finalMap.put("SHENQINGH", row.get("SHENQINGH"));
                    }
                    listFinalMap.add(finalMap);
                }
            }
            count = listFinalMap.size();
        }else if (ConsType.equals("TaskSend")){
            count = rows.size();
            listFinalMap = rows;
        }
        ...
        for (int x = 0; x < listFinalMap.size(); x++) {
            Map<String, Object> row = listFinalMap.get(x);
            for (String key : row.keySet()) {
                String Key = key.toUpperCase();
                if (rowKey.containsKey(Key)) {
                    int colIndex = rowKey.get(Key);
                    Cell writeCell = sheet.getRow(firstRowIndex + x).getCell(colIndex);
                    if (writeCell != null) {
                        Object oValue = row.get(key);
                        String value = "";
                        if (ObjectUtils.isEmpty(oValue) == false) {
    ...
    public byte[] SendYearFeeGetContent(List<Map<String, Object>> listMap, String ConsType) throws Exception {
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        // writeRowsSender(listMap,ConsType);
        HSSFFormulaEvaluator.evaluateAllFormulaCells(book);
        book.write(outputStream);
        byte[] Bx = outputStream.toByteArray();
        outputStream.close();
